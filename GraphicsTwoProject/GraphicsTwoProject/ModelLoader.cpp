...
void ModelLoader::calculateModelVectorsWithFaceNorms()
{
	int faceCount, index;
	GraphicsTwoProject::VertexUVNormTanBi vertex1, vertex2, vertex3;
...
bool ModelLoader::loadMaterialOBJWithFaceNorms(std::shared_ptr<DX::DeviceResources> deviceResources, const char * path)
{
	FILE * file;
	//file = fopen(path, "r");
	errno_t openCheck;
	...
	//Calculate model vectors for modelMaterialFaceVerts
	calculateModelVectorsWithFaceNorms();
